...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		// Credit to TA Xueqing Deng for psuedocode

		// I am not totally certain as to why it does not allow the computer to move. 
		for(DieFace up1 : DieFace.values()) {
			for (DieFace up2 : DieFace.values()) {
				for (DieFace up3 : DieFace.values()) {
					double this_roll = s.rollProb(up1, up2, up3);
					s = s.roll(up1, up2, up3);
					double roll_val = value_rolled_hand(s, depth);
					val = val + roll_val * this_roll;
				}
			}
		}

		//State newState = new State(s);
		//newState = newState.rollState();
		//val = value_rolled_hand(newState, depth);
	...
	static public double heuristic(State s) {
		...
		// Credit to TA Xueqing Deng for psuedocode
		float blasts_scale = (float) 0.75;
		float no_blasts_scale = (float) 0.90;
		double near_win_scale = 0.90;
		int eff_comp_brains_eaten = s.comp_brains_eaten;
		int eff_user_brains_eaten = s.user_brains_eaten;

		if(s.current_player == Turn.computer) {
			if(s.current_choice == Choice.roll) {
				if(s.blasts_collected > 0) {
					eff_comp_brains_eaten = eff_comp_brains_eaten +
							Math.round(blasts_scale * ((float) s.brains_collected));
				} else {
					eff_comp_brains_eaten = eff_comp_brains_eaten +
							Math.round(no_blasts_scale * ((float) s.brains_collected));
				}
			} else {
				eff_user_brains_eaten = eff_user_brains_eaten + s.brains_collected;
			}
		} else {
			if(s.current_choice == Choice.roll) {
				if(s.blasts_collected > 0) {
					eff_user_brains_eaten = eff_user_brains_eaten +
							Math.round(blasts_scale * ((float) s.brains_collected));
				} else {
					eff_user_brains_eaten = eff_user_brains_eaten +
							Math.round(no_blasts_scale * ((float) s.brains_collected));
				...
				eff_comp_brains_eaten = eff_comp_brains_eaten + s.brains_collected;
			}
		}

		if ((eff_comp_brains_eaten > s.brains_to_win) || (eff_user_brains_eaten > s.brains_to_win)) {
			if (eff_comp_brains_eaten > eff_user_brains_eaten + 1) {
				value = near_win_scale * State.win_payoff;
			} else {
				if (eff_user_brains_eaten > eff_comp_brains_eaten) {
					value = -(near_win_scale * State.win_payoff);
				} else {
					value = 0.0;
				}
